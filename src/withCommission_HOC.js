...
<Component count={count*3}/>
  ...
  function  WithCommissionHOC({ closeExchangeFeedback })  {
  // const [count, setCount] = useState(2);
     const { count, handleIncrement, handleDecrement } = useCounter();

return (Component ) => {

  return ({ onChange, commission, ...props }) => {
...
// if (!isOpen) return <div>feedback:{closeExchangeFeedback} </div>;
  ...
  <Component count={count*1000*count2*commission} />
...
export default WithCommissionHOC ;
